...
"Horizon X5 ASIC Evaluation board")
    UDC_USB2=35300000.usb
    UDC_USB3=35100000.usb

    UDC=$UDC_USB2
    UDC_ROLE=/dev/null # Not generic
    ;;
"Horizon Robotics X5 SOM board")
    UDC_USB2=35300000.usb
    UDC_USB3=35100000.usb
...
"Horizon Robotics X5 EVB board")
    ...
    UDC_ROLE=/dev/null # Not generic
    ;;
"Horizon Robotics X5 EVB-V2 board")
    UDC_USB2=35300000.usb
    UDC_USB3=35100000.usb
...
create_frame() {
	...
	ndir=functions/$FUNCTION/streaming/$FORMAT/$NAME
	mkdir -p $ndir
	if [[  -n $SUBTYPE && $SUBTYPE == "h264" ]]; then
		echo "H264" > $ndir/guidFormat		# H264 or HEVC
	...
	echo $(( $WIDTH * $HEIGHT * 2 * 8 * 30)) > $wdir/dwMinBitRate
	echo $(( $WIDTH * $HEIGHT * 2 * 8 * 30)) > $wdir/dwMaxBitRate

	if [[  -n $SUBTYPE && $SUBTYPE == "h264" ]]; then
		# echo 0 > $wdir/dwBytePerLine		# permission denied??
		echo $(( $WIDTH * $HEIGHT * 1 )) > $wdir/dwMaxVideoFrameBufferSize

		# *!*WARNING:  if bFrameIntervalType is 1 then dwMinBitRate should equal dwMaxBitRate
		echo $(( $WIDTH * $HEIGHT * 1 * 8 * 30)) > $wdir/dwMinBitRate
		echo $(( $WIDTH * $HEIGHT * 1 * 8 * 30)) > $wdir/dwMaxBitRate
	fi
...
delete_uvc() {
	...
	# rm uncompressed(yuyv)
	rmdir functions/$FUNCTION/streaming/uncompressed/u/*/
	rmdir functions/$FUNCTION/streaming/uncompressed/u

	# rm uncompressed(h264)
	rmdir functions/$FUNCTION/streaming/uncompressed/f/*/
	rmdir functions/$FUNCTION/streaming/uncompressed/f
